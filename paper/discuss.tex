...
of the metadata of an experiment. Other information can nevertheless
...
durations, as we have done in the code listings in the Supplementary
...
such as Buckingham's $\pi$-theorem. 

Furthermore, in our formulation,
...
Hierarchical probabilistic notation \citep{Gelman2006}
permits the construction of flexible statistical models for the
directly observed data. 

This means that CoPE in principle can  such probabilistic models into powerful data analysis tools
that can be accessed from within the CoPE formalism, 

To make this work in the context of CoPE
requires a mechanism for defining new elementary probability
distributions for signals, events and durations in terms of random
processes. 



The derivation of such data analysis tools can largely be automated,
as is done in the statistical packages WinBUGS \citep{Gilks1994} or
Mlwin \citep{mlwin}, or by compiling descriptions of probabilistic
models in CoPE to the specification format used by the AutoBayes
\citep{AutoBayes} system to generate efficient code for statistical inference.


This analysis workflow could be seamlessly integrated into an
implementation of CoPE, such that not only the hierarchical model
structure could be defined in terms of temporal contexts but also that
the parameters estimated from a statistical inference tool such as
AutoBayes are tagged with the appropriate temporal contexts.

 


This methodology can be used to quantify different aspects of
uncertainty in the measurements, taking into account all available
information, while largely avoiding ad-hoc transformations of
data. Integrating well-developed statistical tools with data
acquisition and manipulation within CoPE would create a powerful
platform for validating inferences drawn from physiological
experiments.
...
The full code for the locust experiment, including relevant metadata,
...
The full code for the Zebrafish experiments, including relevant
