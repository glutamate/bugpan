...
experimental science, here demonstrated for physiology. \underline{We propose
...
provenance \cite{Pool2002}. \underline{In addition, the tradition of
...
\underline{There are clear advantages to sharing primary data from scientific
...
ontology for physiology. \underline{that address the technical barriers to data
...
\underline{Existing software packages used for the acquisition and analysis of
...
\underline{Previous work on scientific knowledge representation has suggested
...
, but remain informal. We suggest
...
\underline{In terms of representing experimental contexts, here we need make no
...
early experiments were conducted, when manipulated
in a later study , influenced key
...
\underline{Using the calculus of physiological evidence, both stimuli and
...
abstractions. \underline{Our approach also makes the conditions of the recording
...
\underline{The use of a formal language for formulating experiments does not preclude the
...
\underline{Graphical user interfaces can also be an efficient way of
...
\underline{We have used the word ``evidence'' to mean direct observations and
...
\underline{A more intriguing possibility is to build statistical models for the
directly observed data, and to use nested
durations to describe a hierarchical organisation 
...
of physiology, this could be achieved by augmenting a BUGS-like
 language with constructors and distributions for
...
checker \cite{DeBruijn1968, Harrison2009}. It is of course not possible to verify
...
likelihood of the observed data.\underline{There has been substantial
  progress in \emph{automation} in the experimental sciences . In
  contrast, there has been almost no work in algorithmic
  \emph{verification} \, which is a separate but overlapping
  application of computers to science. Nevertheless, if such
  verification is feasible it may lead to a radical change in the way
  scientific research is conducted and communicated.} It is likely
that at least some aspects of validation in physiology can be achieved
with conservative extensions of
