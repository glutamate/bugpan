...
for instance by accessing statistical packages WinBUGS
\citep{Gilks1994} or Mlwin \citep{mlwin}. Alternatively it would be
possible to compile descriptions of probabilistic models in CoPE to
the specification format used by the AutoBayes \citep{AutoBayes}
system, which would then be used to generate efficient code for
statistical inference. When run, this code would return data to
CoPE. This analysis workflow could be integrated seamlessly into an
...
like AutoBayes with data acquisition and manipulation within CoPE
would create a powerful platform for validating inferences drawn from
