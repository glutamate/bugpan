...
Reproducible research requires procedures to be described in
sufficient detail such that the experiments can be replicated and the
conclusions can be reconstituted from the unprocessed observations
\citep{Fomel2009}. However, as experiments and analysis are becoming
increasingly complex and based on computer code, the feasibility of
independent verification and replication has -- in practice --
gradually been undermined. Several studies indicate that full
disclosure of raw data and code for experiments and analysis does not
guarantee reproducibility.  For instance, only in 8 out of 16 studies
subject to this requirement and published in \emph{Nature Genetics}
could the findings be replicated at all, and only in 2 of those
without discrepancies \citep{Ioannidis2008}. Low reproducibility even
with available code and data has been reported elsewhere
\citep{Baggerly2009, McCullough2007}. It is not realistic for peer
reviewers to verify that computer code correctly implements a intended
mathematical specification; any inconsistencies can undermine the
...
algebraic manipulation and reasoning about the experiments and
analysis procedures.
