...
autoSpikes sigNm = do
  ...
  ((_,thresh):_) <- durations "threshold" (1::Double)
  io $ initGUI
  Session bdir _ <- getSession
  ...
  let waveforms =  limitSigs' (-0.0005) 0.0005 $ around putatives $ sigs
  let alignWaveforms = --limitSigs' (-0.0006) 0.0006 $ 
                         unjitter $ 
                         alignBy (centreOfMass . ((square . square) <$$>) ) $ 
                         waveforms 
  let alignedEvents = shiftEach (map (negate . (+0.0005). fst) $ sigStarts alignWaveforms) putatives
