...
epspSigs sess = do 
  nms <- fmap catMaybes $ inEverySession $ whenContinues sess $ do
     ...
     let noiseSigs = take 20 $ limitSigs' (-0.06) (-0.01) $ around (spikeg) $ vm
     let epspSigs = during (durAroundEvent (0.01) 0.04 spikeg) vm 
...
measAmps sess = runRIO $ do
  ...
  forM_ sigs $ \sig@(Signal dt t0 _) -> do
      ...
      vsamples<- runAdaMetRIO 2000 True froampar (posteriorSigV wf invDetails sig)
